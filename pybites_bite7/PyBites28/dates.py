...
def convert_to_datetime(date_str):
    ...
    return datetime.strptime(date_str[5:22], "%d %b %Y %H:%M")


def get_month_most_posts(dates):
    """Receives a list of datetimes and returns the month (format YYYY-MM)
       that occurs most"""
    strings = [date.strftime("%Y-%m") for date in dates]

    coll = collections.Counter(strings)

    most_frequent = coll.most_common(1)
